...
cls
color 07
title  Microsoft Activation Scripts AIO 1.6  汉化: Ma5老师
mode 76, 30
...
echo:
echo:
call :_color2 %_Yellow% "         Windows/Office 三合一万能激活工具[AIO 1.6]   " %_Green% "汉化: Ma5老师"
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "         " %_Green% "根据菜单选项单击数字键 [1,2,3,4,5,6,7,8] :"
choice /C:12345678 /N
set _erl=%errorlevel%
...
echo:
echo:             [4] 更改 Windows 版本
echo:             __________________________________________________      
echo:                                                                     
echo:             [5] 返回主菜单
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "根据菜单选项单击数字键 [1,2,3,4,5] :"
choice /C:12345 /N
set _erl=%errorlevel%
...
%psc% $ExecutionContext.SessionState.LanguageMode 2>nul | find /i "Full" 1>nul || echo PowerShell 未正确响应
) else (
echo:
...
echo:
echo 注意：如果您需要 KMS38 激活服务器核心/Acor 版本，那么
echo       从MAS单独文件版本中提取
)
...
if %~z0 GEQ 500000 (set "_exitmsg=返回") else (set "_exitmsg=退出")
...
cls
mode 102, 34
if %_gent%==1 (set _title=title  Generate HWID GenuineTicket.xml) else (set _title=title  数字 HWID 激活)
...
set _gent=0
%eline%
echo 文件 '%Systemdrive%\GenuineTicket.xml' 已经存在。
if %_unattended%==0 (
echo:
...
echo:
echo 在MAS的附加部分中选中“更改版本选项”。
) else (
echo 评估版无法激活。 下载 ^& 安装完整版的 Windows 操作系统。
echo:
...
echo 在 SKU 值中发现差异 - WMI:%wmiSKU% Reg:%regSKU%
echo 重新启动系统并重试
goto dk_done
)
goto dk_done
...
set error=1
call :dk_color %Red% "检测互联网连接            [连接到 licensing.mp.microsoft.com 失败]"
)
)
...
if defined serv_csts echo 启用禁用服务              [成功] [%serv_csts%]
if defined serv_cste call :dk_color %Red% "Enabling Disabled Services              [失败] [%serv_cste%]"

if not "%regSKU%"=="%wmiSKU%" (
set error=1
call :dk_color %Red% "正在检测 WMI/REG SKU                    [发现差异 - WMI:%wmiSKU% Reg:%regSKU%] [重启系统]"
)
...
call echo 检查已安装的产品密钥          [Partial Key - %%_partial%%] [%_channel%]
)
...
call :dk_refresh
call :dk_channel
call echo 安装通用产品密钥          [%key%] [%%_channel%%] [成功]
) else (
call :dk_color %Red% "正安装通用产品密钥         [%key%] [失败] !error_code!"
)
)
...
call :dk_color %Red% "将所需文件提取到临时      [%temp_%] [失败]"
call :dk_color %Magenta% "最有可能的防病毒软件正在干扰该过程"
call :dk_color %Magenta% "使用 MAS 单独文件版本"
goto :dl_final
)
...
call :dk_color %Red% "提取文件验证失败，中止..."
goto :dl_final
)

echo 将所需文件提取到Temp      [%temp_%] [成功]
...
call :dk_color %Red% "正在创建改良的聚集状态        [失败]中止..."
goto :dl_final
)
...
call :dk_color %Red% "正在创建改良的聚集状态         [失败] [Hash Not Matched] 中止..."
goto :dl_final
) else (
echo 创建修改后的Gatherosstate [成功]
)
...
call :dk_color %Red% "正在清除 ClipSVC Licences               [失败]"
) else (
echo 清理ClipSVC许可证              [成功]
)
)
...
call :dk_color %Red% "正在删除 一个Volatile注册表项            [失败]"
call :dk_color %Magenta% "重新启动系统，将自动删除此注册表项"
) || (
echo 正在删除 一项Volatile注册表            [成功]
)
...
call :dk_color %Red% "正在删除一个注册表项                    [失败][%_ident%]"
) || (
echo 正在删除 一项注册表                   [成功] [%_ident%]
)
)
...
%_noxml% (
call :dk_color %Red% "正在创建GenuineTicket.xml            [失败] [%pfn%]"
goto :dl_final
)

if %_lock%==1 (
find /i "clientLockboxKey" "%temp_%\GenuineTicket.xml" >nul && (
echo 生成 GenuineTicket.xml [成功] [%pfn%]
) || (
call :dk_color %Red% "正在创建GenuineTicket.xml            [失败] [%pfn%]"
call :dk_color %Red% "downlevelGTkey Ticket 被创建，中止..."
goto :dl_final
)
) else (
echo 生成 GenuineTicket.xml [成功] [%pfn%]
...
call :dk_color %Red% "正在复制 GenuineTicket.xml to %Systemdrive%\        [失败]"
) else (
call :dk_color %Green% "将 GenuineTicket.xml 复制到 %Systemdrive%\ [成功]"
)
goto :dl_final
)
...
call :dk_color %Red% "正在复制 Ticket to ClipSVC Location      [失败]"
)
...
%_xmlexist% (
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "正在安装 GenuineTicket.xml            [Failed With ClipSVC Service Restart Method]"
)
)
...
if defined _perm (
set activ=1
call :dk_color %Green% "%winos% 已经永久激活！"
goto :dl_final
)
...
set "_unsup=call :dk_color %Magenta% "在写本程序时，此产品不支持 HWID 激活。""
...
if not defined notworking if defined key if not defined pkey call :dk_color %Magenta% "重新启动系统并重试/检查 MAS 扩展选项中的故障排除步骤"
)
...
if exist "%temp_%\.*" rmdir /s /q "%temp_%\" %nul%
if exist "%temp_%\" (
call :dk_color %Red% "正在清除临时文件                    [失败]"
) else (
echo 清理临时文件                     [成功]
)

if %osSKU%==175 (
call :dk_color %Red% "ServerRdsh 版本不正式支持在非Azure平台上激活。"
)
...
echo:
echo 正在启用 %%# 服务...
if /i %%#==sppsvc  sc config %%# start= delayed-auto %nul% || echo 失败
if /i %%#==WinMgmt sc config %%# start= auto %nul% || echo 失败
)
sc start %%# %nul%
...
sc start %%#
echo:
call :dk_color %Red% "无法启动 [%%#] 服务，其余过程可能需要很长时间..."
)
)
...
if %token%==0 (
set error=1
call :dk_color %Red% "正在检测 SPP tokens.dat                 [没有找到]"
)
...
set error=1
call :dk_color %Red% "正在检测 Rearm                          [需要重启系统]"
)

reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ClipSVC\Volatile\PersistedSystemState" %nul% && (
set error=1
call :dk_color %Red% "正在检测 ClipSVC                        [需要重启系统]"
)
...
if %_wsh% EQU 0 (
set error=1
call :dk_color %Gray% "检查 Windows 脚本Host     [已禁用]"
)
...
if not defined applist (
set error=1
call :dk_color %Red% "正在检测 WMI/SPP                        [没有反应] [%e_wmispp%]"
)
...
if defined _sppint (
call :dk_color %Red% "正在检测 SPP Interference In IFEO       [找到] [卸载 KMS 激活器(如果有)]"
set error=1
)
exit /b
...
%eline%
echo 检测到不支持的操作系统版本
echo 支持Windows 10/11/Server、build 14393 及更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "单击数字键选项 [1,2,3,4]"
choice /C:1234 /N
set _el=!errorlevel!
if !_el!==4  exit /b
...
cls
echo:
call :dk_color2 %_White% " " %_Green% "KMS38 保护:"
echo:
echo  它停止 180 天 KMS 激活并替换成 KMS38 激活
echo  保护需要更改注册表项的权限
echo:
echo  如果您打算使用KMS_VL_ALL或MAS的KMS激活Office，
echo  那么您不需要启用此保护
echo  有关更多信息，请查看自述文件
echo:
echo:
...
cls
mode 102, 34
if %_gent%==1 (set _title=title  Generate KMS38 GenuineTicket.xml) else (set _title=title  KMS38 激活)
%_title%
...
if not defined applist (
echo 获取激活 ID 出错导致获取密钥失败。
echo:
echo 当以下服务没有正确响应或系统有其他问题时，可能会出现此错误。
echo Windows 管理规范 [WinMgmt]，软件保护平台 [sppsvc]
echo:
call :dk_color2 %Red% "Error Found In:" %_White% " %e_wmispp%"
echo:
echo 检查MAS附加选项中的故障排除步骤。
goto dk_done
)
echo [%winos% ^| %winbuild% ^| SKU:%osSKU%]
echo 在支持的产品列表中找不到该产品。
...
if defined a_cor (
%eline%
echo Server Cor 版本没有 clipup.exe 文件。
echo:
echo 在此类版本上使用 MAS 单独文件版本进行 KMS38 激活。
goto dk_done
)
...
if not defined app (
call :dk_color %Red% "正在检测 Activation ID                  [失败]"
)
...
call :dk_color %Red% "添加指定 KMS 主机                [略过] [没有找到激活ID]"
)
...
%_noxml% (
call :dk_color %Red% "正在创建GenuineTicket.xml            [失败] 中止..."
goto :k_final
) else (
...
call :dk_color %Magenta% "重新启动系统并重试/检查 MAS 扩展选项中的故障排除步骤"
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Gray% "锁定注册表以保护 KMS38 [成功]"
) || (
call :dk_color %Red% "锁定注册表以保护 KMS38     [失败]"
)
)
...
REM call :dk_color %Red% "正在清除 ClipSVC Licences               [失败]"
REM ) else (
REM echo 清理ClipSVC许可证              [成功]
REM )
...
REM call :dk_color %Red% "正在删除 一个Volatile注册表项            [失败]"
REM call :dk_color %Magenta% "重新启动系统，将自动删除此注册表项"
REM ) || (
REM echo 正在删除 一项Volatile注册表            [成功]
REM )
...
REM call :dk_color %Red% "正在删除一个注册表项                    [失败][%_ident%]"
REM ) || (
REM echo 正在删除 一项注册表                   [成功] [%_ident%]
REM )
...
%nceline%
echo 检测到不支持的操作系统版本
echo 支持Windows 7/8/8.1/10/11 及其等效服务器。
goto Done
)
...
call :_color2 %_White% "              [3] 激活 - 二者           " %_Red% "[调试模式]"
)
echo.
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "按选项单击数字键 [1,2,3,4,5,6,7]"
choice /C:1234567 /N
set _el=%errorlevel%
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo 无法检查 Windows 的 KMS 激活 ID^(s^)。&echo 检查 MAS extras 部分中的故障排除。&call :CheckWS
    exit /b
    )
  )
)
...
echo.
echo Microsoft Office 通过订阅激活。
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project 已通过订阅激活。
)
if %sub_vis% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio 通过订阅激活。
)
...
if defined _tskinstalled echo 找到续订任务，系统中保留在线KMS IP.
if defined _oldtsk echo 找到续订任务，系统中保留在线KMS IP.
...
(set msg1=echo 再试一次，如果问题仍然存在，请使用 a^
&echo 不同的 Internet 连接或使用此离线 KMS 激活器^
...
call :_color %_Red% "由于 Internet 受限或没有 Internet，无法测试 KMS 服务器"
echo.
%msg1%
exit /b
)
...
set error_=9
echo 无法完全清除 KMS 缓存。
reg query "HKLM\%SPPk%\%_wApp%" /s 2>nul | findstr /i "127.0.0.2" >nul && echo 很可能，KMS38 激活被锁定
) || (
echo 清理KMS缓存成功
)
exit /b
...
echo.
call :_color %Gray% "确保至少从上一级菜单运行激活一次"
echo __________________________________________________________________________________________
)
...
(
echo   此脚本的用途是使用在线 KMS 激活/更新您的 Windows/Office 许可证
echo:
...
echo:                          
echo:             [4] 重建许可令牌
echo:             [5] 清除 Office vNext 许可证
echo:             ___________________________________________________      
echo:                                                                      
echo:             [6] 解决方法：office不是正版标识
echo:             [0] 返回
echo:       _______________________________________________________________
echo:          
call :_color2 %_White% "            " %_Green% "单击选项中的数字键 :"
...
echo:
call :_color %Red% "未检测到桌面位置无法在桌面上复制日志"
goto :at_back
)
...
echo:
call :_color %Gray% "CBS 和主要提取的日志被复制到桌面上的 ACT Logs 文件夹中"
goto :at_back
...
echo %line%
echo:    
echo      此选项将清除 Office vNext ^(subscription^) 许可证
echo:
echo      您需要在以下情况下使用此选项，
echo          - 由于现有订阅许可证，KMS 选项未激活 Office
echo          - KMS 选项已激活Office，但Office激活页面未显示已激活
echo:   
echo %line%
echo:
choice /C:29 /N /M ">    [9] 继续 [2] 返回 : "
...
echo       它有助于解决激活问题
echo:
call :_color2 %_White% "      - " %Magenta% "此选项将会,"
call :_color2 %_White% "        " %Magenta% "- 取消Windows 和 Office激活，您需要重新激活"
call :_color2 %_White% "        " %Magenta% "- 卸载 Office 许可证和密钥"
call :_color2 %_White% "        " %Magenta% "- 清除 SPP-OSPP data.dat、tokens.dat、cache.dat"
call :_color2 %_White% "        " %Magenta% "- 触发 Office 的修复选项”
echo:
...
call :_color %Green% "tokens.dat 文件已成功重建"
)
...
sc qc osppsvc %nul% || (
echo:
call :_color %Magenta% "未安装基于 OSPP 的 Office"
call :_color %Magenta% "跳过重建 OSPP 令牌"
goto :cleanclipsvc
)
...
echo:
call :_color %_Yellow% "将弹出一个窗口，在该窗口中您需要选择修复选项..."
...
cls
color 07
title  安装 Windows Retail/OEM/MAK 秘钥
set _unattended=0
...
%eline%
%psc% $ExecutionContext.SessionState.LanguageMode 2>nul | find /i "Full" 1>nul || (
echo PowerShell 未正确响应
echo:
)
echo 找不到 [%winos% ^| 的 HWID 密钥 SKU:%osSKU% ^| %winbuild%]
echo 确保您使用的是脚本的更新版本
echo:
if not "%regSKU%"=="%wmiSKU%" (
echo 在 SKU 值中发现差异 - WMI:%wmiSKU% Reg:%regSKU%
...
%line%
echo:
echo 安装 [%winos% ^| SKU:%osSKU% ^| %winbuild%] %_chan%
echo [%key%]
%line%
echo:
if not "%regSKU%"=="%wmiSKU%" (
...
call :dk_color %_Green% "按[1]继续 按[2]返回"
choice /C:21 /N
...
if not defined applist echo 无反应: %e_wmispp%
)
%line%
...
%eline%
echo 检测到不支持的操作系统版本
echo 仅支持 Windows 10/11/Server Build 10240 以上的版本
goto ced_done
)
...
if %error_code% NEQ 0 (
%eline%
echo slmgr /dlv 没有响应 %error_code%
goto ced_done
)
...
if defined _target (
for %%# in (%_target%) do (
echo %%# | findstr /i "国家特定云版" %nul% || (if defined _ntarget (set "_ntarget=!_ntarget! %%#") else (set "_ntarget=%%#"))
)
)
...
%line%
echo:
call :dk_color %Gray% "你可将当前版本[%osedition%]更改为下列版本"
%line%
echo:
...
if %_changepk%==1 (
call :dk_color %Green% "如果升级窗口中出现错误，您可以放心地忽略"
call :dk_color %Red% "但在这种情况下，您必须手动重新启动系统"
echo:
